...
**Where to read from:** kb keeps project-authored skills and Skills CLI-managed project skills under **`<repo>/.agents/skills/<id>/SKILL.md`** (same folder as `kb-context`, `kb-rpc`, etc.). Use `mise run skill install` to restore external project skills from `skills-lock.json`. Global companions stay under `$HOME/.agents/skills/`.
