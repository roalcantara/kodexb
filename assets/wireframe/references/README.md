...
PowerToys Run is the definitive visual reference for kb's list + detail layout.
It is Windows-only — no competitive or ethical conflict with kb (macOS).

### Why PowerToys Run

- Keyboard-first, search-centred, information-dense — identical goals to kb
- Two-panel layout (list left, detail right) already matches kb's wireframe
...
2. **`/Users/roalcantara/Work/bun/kb/DESIGN.md`** — exact palette tokens (Andromeda Void)
