...
Normalized inventory for maintainer review: **74** capabilities (**41** Elysia, **33** Electrobun) in `inventory.yml`, generated **2026-05-27**.

| Metric                                 | Count |
| -------------------------------------- | ----- |
| `current_usage.status: used`           | 13    |
| `current_usage.status: partial`        | 16    |
| `current_usage.status: not-used`       | 30    |
| `current_usage.status: not-applicable` | 13    |
| `current_usage.status: unknown`        | 2     |
| `priority: must-have`                  | 4     |
| `priority: nice-to-have`               | 1     |
| `priority: postponed`                  | 6     |
| `priority: meh`                        | 3     |
| `priority: undecided`                  | 60    |
...
- Upstream docs were refreshed for a coverage addendum on **2026-05-22** and an Electrobun corpus audit on **2026-05-27**; see [`electrobun-corpus-audit/report.md`](../electrobun-corpus-audit/report.md).
...
1. **Partial adoptions** — mixed upstream vs app usage: `elysia-typebox`, `elysia-lifecycle`, `elysia-config`, `elysia-plugin-dedup`, `electrobun-code-signing`, `electrobun-platform`, `electrobun-utils`, `electrobun-events`, `electrobun-global-shortcut`, `electrobun-bundling-cef`, `electrobun-screen`, `elysia-best-practices`, `electrobun-compatibility`, `electrobun-build-configuration`, `electrobun-cli-args`, `electrobun-getting-started`.
2. **Unknown evidence** — needs targeted code confirmation: `elysia-typescript-performance`, `electrobun-global-properties`.
3. **Upstream-only / policy conflicts** — examples include `elysia-cors`, `elysia-jwt`, `elysia-bearer`, `elysia-html`, `elysia-standard-schema`, `elysia-integration-drizzle`, `elysia-integration-ai-sdk`, `electrobun-webgpu`, `electrobun-wgpu-tag`, `elysia-cookies`, `elysia-deploy-patterns`, `elysia-graphql-apollo`.
...
Maintainer tables derived from `inventory.yml` (33 items). Detailed narrative notes remain in YAML `notes` and `current_usage.notes`. Adoption tiers: [`electrobun-corpus-audit/report.md`](../electrobun-corpus-audit/report.md).
...
| ✔    | electrobun | native-ui    | GlobalShortcut                        | [bun][49]                            | Partial (H)       | Undecided | Quick show/hide without focusing the webview first.                             | No unregisterAll on shutdown; see corpus audit.                                     | —                                                                                      |
...
|      | electrobun | lifecycle    | App and window Events API             | [events][53]                         | Partial (H)       | Undecided | before-quit supports async cleanup and quit cancel.                             | Linux system quit may skip before-quit; kb uses blur only today.                    | Save window state on before-quit via Electrobun Events API.                            |
...
|      | electrobun | window       | Draggable regions                     | [draggable-regions][63]              | Not used (H)      | Undecided | Native drag regions for frameless windows.                                      | App uses RPC-based window drag instead.                                             | —                                                                                      |
...
|      | electrobun | build        | Bundling CEF                          | [bundling-cef][67]                   | Partial (H)       | Undecided | CEF enables CDP debugging and cross-platform renderer consistency.              | Increases artifact size; opt-in via ELECTROBUN_RENDERER=cef.                        | Decide whether CEF bundling is needed for Windows/Linux release targets.               |
| ✔    | electrobun | build        | Application icons                     | [application-icons][68]              | Used (H)          | Undecided | Required packaging polish is already wired.                                     | —                                                                                   | —                                                                                      |
|      | electrobun | display      | Screen (displays and cursor)          | [bun][49]                            | Partial (H)       | Undecided | Foundation for cursor-adjacent window placement.                                | Primary display only; no getCursorScreenPoint yet.                                  | Resolve display from cursor before initial BrowserWindow frame.                        |
|      | electrobun | build        | BuildConfig runtime API               | [build-config][69]                   | Not used (H)      | Undecided | Runtime renderer availability from build.json.                                  | Static config in electrobun.config.ts suffices today.                               | Log BuildConfig at startup when LOG_LEVEL=verbose.                                     |
|      | electrobun | webview      | BrowserView navigation rules          | [browser-view][43], [events][53]     | Not used (H)      | Undecided | Required security gate before external URLs.                                    | Production loads views:// only.                                                     | Add navigation allowlist if preview loads http(s) URLs.                                |
|      | electrobun | gpu          | WGPU tag (renderer)                   | [wgpu][70]                           | N/A (H)           | Undecided | —                                                                               | Irrelevant to knowledge-management UI.                                              | —                                                                                      |
|      | electrobun | onboarding   | Getting started guides                | [quick-start][71], [hello-world][72] | Partial (H)       | Undecided | ELECTROBUN.md maps kb wiring to onboarding docs.                                | Onboarding alignment only; no code adoption.                                        | —                                                                                      |
...
[69]: https://blackboard.sh/electrobun/docs/apis/build-config
[70]: https://blackboard.sh/electrobun/docs/apis/wgpu
[71]: https://blackboard.sh/electrobun/docs/guides/quick-start/
[72]: https://blackboard.sh/electrobun/docs/guides/hello-world/
