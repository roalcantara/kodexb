...
**References:** [Cucumber Gherkin reference](https://cucumber.io/docs/gherkin/reference/), [Screenplay fundamentals](https://serenity-bdd.github.io/docs/screenplay/screenplay_fundamentals), kb `design.md#screenplay-conventions`.
...
| `e2e/steps/sync_frecency.steps.ts`        | Frecency preserved across sync           |
...
| I have opened some items several times             | Given   | `SeedFrecencyLeader` + `RecordUsefulVisitsFor`  | N     |
| I sync my files from disk                          | When    | `RunSync` (⌘P → Sync)                           | N     |
| the items I use most still appear before the ones I rarely open | Then | `EntryOrdering.ranksAbove(most, rare)` | N     |
...
| `sync_frecency`        | Frequently opened items keep place…    | `sync_frecency.frequently_opened_items_keep_their_place_after_sync` |
...
Reject combined action+assertion steps (see `design.md#screenplay-conventions`).
