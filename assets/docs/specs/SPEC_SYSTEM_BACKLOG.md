...
Inventory: `mise run audit rogue-refs` (diagnostic; see [`tools/governance/policies/rogue_refs.script.ts`](../../../tools/governance/policies/rogue_refs.script.ts)).
...
## P2-archive-rename (deferred)

`git mv assets/docs/specs/` → `assets/docs/archive/` when:

- Permanent docs no longer link into the tree (except allowlisted tools).
- `tools/governance/specs/library_manifest.script.ts` and `import_legacy.script.ts` updated.
- ast-grep ignores and opencode commands swept.

Goal: path name signals **OLD**; agents use guides + catalog, not directory search.
