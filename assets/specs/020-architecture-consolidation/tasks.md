...
- [ ] **T103** `TaskView` → core (0 core→`@shared/rpc`) — *gate:* ARCH-1 AC1
- [ ] **T104** `BindingRef` one def in `@shared/rpc`; 0 `shared→core` — *gate:* ARCH-1 AC2/AC6
...
- **Body:** Track structuralSuppressionCount, maxFileLoc, oversizedFileCount;
...
- **Body:** Define TaskView in core; core modules import from core, not
...
- **Body:** One BindingRef definition + schema in @shared/rpc, imported by core
...
- **Body:** wouldCreateCycle + create-task tag normalization become pure core
...
- **Body:** taskIsOverdue/taskIsBlocked merge into core task_views predicates;
...
- **Body:** taskView/priority/entryType unions derive from core tuples via
...
- **Body:** ListStats/RpcDbStats/sync shapes gain TypeBox schemas (Static<>);
...
- **Body:** Extract query/task/sync/config/lifecycle services; App becomes a
...
- **Body:** git mv app_* into lib/{list,sync,task,preview,shell}/ dropping the
...
- **Body:** Separate Eden transport from the per-endpoint facade; client below
...
- **Body:** Named contracts replace the ListPageShell p bag; break up ListMain;
...
- **Body:** appendTypeFacetRows + list reads iterate ENTRY_TYPE_VALUES over
...
- **Body:** One coordinator owns overlay open/close + stacking; remove scattered
...
- **Body:** Extract backdrop/header/focus-trap/dismiss backing sync modal, task
...
- **Body:** SyncModalModel and siblings move to .types/.model; no type-only
...
- **Body:** git mv list_*/entry_action_* to drop the folder-implied prefix; add
...
- **Body:** Separate primitives (chips/badges/markdown) from sync-feature
...
- **Body:** Role suffixes for actions (+ panel/ subfolder); single-source the
...
- **Body:** Split shell_hooks by concern into vocabulary-valid modules placed by
...
- **Body:** Ratchet baseline (suppressions→0), record closeout metrics, fix
  duplicate P3-7, register catalog key, full spec-ready green.

## Closeout
- [ ] **T121** Ratchet baseline (suppressions→0), closeout metrics, fix duplicate P3-7, catalog key, full `mise run spec ready` — *gate:* DoD 1–7
