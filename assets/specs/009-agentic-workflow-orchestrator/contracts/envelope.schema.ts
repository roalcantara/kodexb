...
export const EvidenceEntry = Type.Object({
  kind: Type.Union([
    Type.Literal('mise_task'),
    Type.Literal('hk_profile'),
    Type.Literal('artifact'),
    Type.Literal('checklist_marker')
  ]),
  ref: Type.String({ description: 'task name, profile name, file path, or marker id' }),
  expected: Type.Optional(Type.String({ description: 'expected exit signal or content hash' }))
})
