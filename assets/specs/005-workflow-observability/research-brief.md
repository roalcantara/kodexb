...
4. **Recommendation:** Adopt a **kb-native JSONL audit log** as MVP (Option A in §3), defer OpenTelemetry to v0.15+, and treat `spec-kit-diagram` and `progress-report` as **inspire-only**, not adopt. Rationale: both seed extensions assume `specs/` not `assets/specs/` and target visualization, not audit.
...
13. **The MVP can ship in one PR.** Estimated touch list: 1 new module (`workflow_run.script.ts` ~200 LOC), 3-4 emit sites in the orchestrator, 1 new spec dir under `assets/specs/005-workflow-observability/`, ~10 new tests.
...
| -------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| Phase markers (analyze-plan/tasks, implement-done) | `assets/specs/<slug>/checklists/*.md`                                                                    | Empty marker files                  |
...
- **F1. `Quratulain-bilal/spec-kit-diagram-`.** Mermaid lifecycle + Gantt + DAG. Assumes `specs/` (kb uses `assets/specs/`). Three diagram commands; no audit. Useful **inspiration** for our own Mermaid generator that reads our scanner.
...
- **Adopt as-is?** No. **Reasons:**
  - Reads `specs/` not `assets/specs/`; would require a path patch in every command.
...
Land at `assets/specs/005-workflow-observability/`. Normative quartet only per OHW-8: `spec.md`, `plan.md`, `tasks.md`, `handoff.md`. No `research.md` (this brief replaces it; archive at `assets/specs/005-workflow-observability/research-brief.md` if desired).
...
**Definition of done for the research:** maintainer says "go with Path B" (or names another), defaults for §11 are confirmed, and `assets/specs/005-workflow-observability/` does **not** yet exist (we wait for the implementation greenlight to create it).
