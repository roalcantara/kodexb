...
- Q: How do WOBS-2 integration tests obtain a populated feature dir? → A: **`mkdtempSync` scratch fixtures only.** Do not depend on the live state of `assets/specs/003-sync-frecency-preserve/` — its checklist markers may change as 003 progresses through its own lifecycle and would flake tests.
...
| **Read-side CLI**        | The new `mise run spec runs {list                                                                                                                                      | show | tail | prune}` subcommand. Read-only; never mutates `assets/specs/` or `.specify/`. |
