...
- [X] **T4** Adopt `literalUnion` in 5 core schema files (global `Type.Literal` ≤ 12) — *gate:* SRC-1 AC4

## Phase C — SRC-2 single-source payload types
- [X] **T5** Relocate `ENTRY_TYPE_VALUES` to `@shared/constants`, re-export from core — *gate:* SRC-2 AC1
- [X] **T6** Move request-payload schemas to `@shared/rpc/payload_schemas`, re-export from `schemas.ts` — *gate:* SRC-2 AC2
- [X] **T7** Derive 6 payload types via `Static<>`; update CONTRACT NOTE + drift test — *gate:* SRC-2 AC3, AC4, AC5

## Phase D — SRC-3 client call kernel
- [X] **T8** Add `call<T>` in `client.ts`; convert 32 wrappers (0 `.then(unwrap) as Promise`) — *gate:* SRC-3 AC1, AC2, AC3
