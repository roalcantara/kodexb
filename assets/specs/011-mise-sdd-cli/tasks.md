...
- [ ] MSC-BASE-01 Branch `feature/011-mise-sdd-cli`; set `.specify/feature.json` → `assets/specs/011-mise-sdd-cli`.
- [ ] MSC-BASE-02 `mise run spec lint` + `mise run spec gate` on 011 spec (pre-implementation).

---

## Phase 1: Task runner + render mode (MSC-1)

- [ ] MSC-RUNNER-01 Implement `tools/support/lib/cli/task_runner.script.ts` (`runSteps`, `TaskRunReport`, gum spin).
- [ ] MSC-RUNNER-02 Extend `render_mode.script.ts` for TTY auto-pretty / `--raw` forced raw.
- [ ] MSC-RUNNER-03 Co-located `task_runner.script.spec.ts` (TTY mocked, raw line format).
- [ ] MSC-RUNNER-04 Wire `spec gate` as first consumer (lint → trace → gate.sh steps).
...
- [ ] MSC-SPEC-01 Rewrite `mise.toml` `spec` usage to normative tree (plan § command tree).
- [ ] MSC-SPEC-02 Expand `tools/bin/spec.script.ts` dispatch (init, audit subcmds, workflow run, nested handoff).
- [ ] MSC-SPEC-03 Remove top-level `audit` task; merge `tools/bin/audit.script.ts` into spec audit paths.
- [ ] MSC-SPEC-04 `spec init` replaces `feature-init`; update `feature_init.script.ts` entry if needed.
- [ ] MSC-SPEC-05 `spec workflow run` replaces `orchestrated-handoff` name in dispatch + profiles.
- [ ] MSC-SPEC-06 `spec audit docs|feature|security` modules + co-located specs.
- [ ] MSC-SPEC-07 Wire `spec ready` through `task_runner`.
...
- [ ] MSC-TEST-02 Flags: `--feat`, `--unit`, `--e2e`, `--smoke`, `--regression` (mutually exclusive scope).
...
**Checkpoint**: `mise run spec test --feat assets/specs/011-mise-sdd-cli --unit` exit 0.
...
- [ ] MSC-DOCS-05 Update `policy` `EXPECTED_PUBLIC_TASKS` for new task shapes.
