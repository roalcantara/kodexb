...
export class SyncReportsHardCollision implements Answerable {
  ...
  async answeredBy(actor: Actor): Promise<void> {
    await actor.eventually(async () => {
      ...
      const warnings = actor.page.locator('.cmp-sync-modal-summary-warnings')
      await expect(warnings).toBeVisible()
      await expect(warnings).toContainText('hard collision')
      await expect(warnings).toContainText(this.chordHash)
    })
