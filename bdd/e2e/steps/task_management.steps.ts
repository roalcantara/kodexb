...
When('I create a task named {string}', async ({ actor }, name: string) => {
  const expectFailure = actor.recall<boolean>('expectMutationFailure') ?? false
  await actor.attemptsTo(expectFailure ? CreateTask.namedExpectingFailure(name) : CreateTask.named(name))
})

When('I change the task description to {string}', async ({ actor }, text: string) => {
  const expectFailure = actor.recall<boolean>('expectMutationFailure') ?? false
  await actor.attemptsTo(expectFailure ? EditTaskDescription.toExpectingFailure(text) : EditTaskDescription.to(text))
})
