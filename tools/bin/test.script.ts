...
import {
  ...
  resolveTagKey,
  runTaggedTests
} from '../governance/registries/catalog/tag.script.ts'
...
async function main(): Promise<void> {
  ...
  switch (ACTION) {
    ...
    case 'e2e': {
      ...
      if (metricsReport) {
        spawnInherit(['bun', 'tools/metrics/harnesses/e2e-quality/e2e_metrics.script.ts', 'report'], ROOT)
      } else if (metricsCompare) {
        spawnInherit(['bun', 'tools/metrics/harnesses/e2e-quality/e2e_metrics.script.ts', 'compare'], ROOT)
      } else if (writeBaseline) {
        spawnInherit(['bun', 'tools/metrics/harnesses/e2e-quality/e2e_metrics.script.ts', 'write-baseline'], ROOT)
      } else if (smoke) {
