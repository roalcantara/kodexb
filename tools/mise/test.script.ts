...
import {
  ...
  resolveTagKey,
  runTaggedTests
} from '../catalog/tag.script.ts'
import { chdirToRepoRoot } from '../shared/repo_root.script.ts'
import { spawnInherit } from '../shared/spawn_inherit.script.ts'
...
async function main(): Promise<void> {
  ...
  switch (ACTION) {
    ...
    case 'e2e': {
      ...
      if (metricsReport) {
        spawnInherit(['bun', 'tools/e2e/e2e_metrics.script.ts', 'report'], ROOT)
      } else if (metricsCompare) {
        spawnInherit(['bun', 'tools/e2e/e2e_metrics.script.ts', 'compare'], ROOT)
      } else if (writeBaseline) {
        spawnInherit(['bun', 'tools/e2e/e2e_metrics.script.ts', 'write-baseline'], ROOT)
      } else if (smoke) {
