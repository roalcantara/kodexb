 ...
 * Run with: bun tools/dev/preview/server.script.ts (default port 3456; set PORT to override)
 */
  ...
  plugins: [
    {
      ...
      setup(build) {
        build.onResolve({ filter: ELECTROBUN_VIEW_RE }, () => ({
          path: path.join(ROOT, 'tools/dev/preview/mock_electroview.script.ts'),
          namespace: 'file'
        }))
      }
    }
