...
Observability baseline for the **preview server** (`tools/preview/server.ts`):
same HTTP RPC surface the renderer uses via the fetch mock
(`tools/preview/mock_electroview.ts`). This is not a full Electrobun WebView
load test.
...
| ------ | ------------------------------------------------------------------------------------------------------ | ----------------------------- | ---------------------------------------------------------- |
| **P1** | Cold start: spawn `bun tools/preview/server.ts` with `PORT=3457`, time until first `GET /` returns 200 | &lt; 300 ms                 | Snappy dev / preview feedback with CI headroom             |
