...
Observability baseline for the **preview server** (`tools/preview/server.ts`): same HTTP RPC surface the renderer uses via the fetch mock (`tools/preview/mock_electroview.ts`). This is not a full Electrobun WebView load test.

All tasks are defined in [`mise.toml`](mise.toml) at this directory (inline Bun scripts). From the repo root, use the delegating stubs: `mise run perf:run`, etc.

Root stubs export `MISE_TRUSTED_CONFIG_PATHS` (trust nested config) and **`PERF_ROOT`** (absolute repo root). Inline Bun tasks may run with a cwd that is not `tools/benchmarks`, so all paths use `PERF_ROOT` / `repoRoot()` — not `process.cwd()` alone.
...
| ------ | ------------------------------------------------------------------------------------------------------ | ----------------------------- | ---------------------------------------------------------- |
| **P1** | Cold start: spawn `bun tools/preview/server.ts` with `PORT=3457`, time until first `GET /` returns 200 | &lt; 500 ms                   | Snappy dev / preview feedback (Raycast-style bar)          |
| **P2** | FTS: `POST /api/list` with `{ "query": "bun", "limit": 50 }` — p50 / p99 over 100 samples              | p50 &lt; 8 ms, p99 &lt; 16 ms | 16 ms ≈ one frame at 60 fps                                |
| **P3** | Type filter: `POST /api/list` with `{ "types": ["task"], "limit": 50 }` — p50 / p99                    | p50 &lt; 8 ms, p99 &lt; 16 ms | Same frame budget; exercises `AppService.list` filter path |
| **P4** | Throughput: `bunx autocannon` 10 s, 10 connections, `POST /api/list` with FTS body                     | ≥ 500 req/s                   | Sustained RPC load sanity check                            |
| **P5** | Stats: `POST /api/getStats` — p50 / p99 over 100 samples                                               | p50 &lt; 5 ms, p99 &lt; 10 ms | Stats panel should stay lighter than list                  |
