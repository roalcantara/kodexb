...
import { loadSecretsRules } from './rules_loader.script.ts'
...
describe('security foundation', () => {
  ...
  it('file selection can choose changed-only candidates', () => {
    const files = ['README.md', 'tools/a.ts', 'src/a.ts', 'docs/x.txt']
    const mockGit = (args: string[]) => {
      if (args.includes('--staged')) return ['tools/a.ts', 'src/a.ts']
      return []
    }
    const changed = selectCandidateFiles(files, { changedOnly: true, base: 'HEAD~1' }, mockGit)
    expect(changed).toContain('tools/a.ts')
