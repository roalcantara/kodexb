...
const SRC_FIXTURE = path.resolve(import.meta.dir, '../../ops/src/__tests__/fixtures/workflow/smoke-feature')

describe('kit_step_resolver', () => {
  let tempFixtureDir: string

  beforeEach(() => {
    tempFixtureDir = path.join(tmpdir(), `resolver-test-${Math.random().toString(36).slice(2, 9)}`)
    cpSync(SRC_FIXTURE, tempFixtureDir, { recursive: true })
  })
  ...
  it('resolves to a valid canonical stage for fixture', () => {
    const step = resolveNext(tempFixtureDir)
...
describe('R2R — review RETRYABLE_FAILURE rewind', () => {
  ...
  beforeEach(() => {
    tempFixtureDir = path.join(tmpdir(), `r2r-test-${Math.random().toString(36).slice(2, 9)}`)
    cpSync(SRC_FIXTURE, tempFixtureDir, { recursive: true })
    doneFile = path.join(tempFixtureDir, 'checklists/implement-done.md')
    reqFile = path.join(tempFixtureDir, 'checklists/requirements.md')
  })
  ...
  it('rewinds to implement when review envelope indicates RETRYABLE_FAILURE', () => {
    ...
    for (const gate of ['review-spec', 'review-plan', 'review-tasks', 'review-handoff'] as const) {
      clearGate(tempFixtureDir, runId, gate)
    }
    ...
    const step = resolveNext(tempFixtureDir, runId)
  ...
  it('resolves terminal when all stages satisfied', () => {
    ...
    const step = resolveNext(tempFixtureDir, 'test-run')
    expect(step.kind).not.toBe('gate')
  })
})
