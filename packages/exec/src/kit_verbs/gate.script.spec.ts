...
import { KB_KIT_SMOKE_ENV } from '../kit_smoke.script'

describe('kit gate', () => {
  it('dispatch module is importable', async () => {
    const mod = await import('./gate.script')
  ...
  it('smoke mode skips gate.sh', async () => {
    ...
    const { run } = await import('./gate.script')
    expect(run([])).toBe(0)
...
describe('kit pr-prep', () => {
  it('dispatch module is importable', async () => {
    const mod = await import('./pr_prep.script')
    expect(typeof mod.run).toBe('function')
  })
})
...
describe('kit pr-open', () => {
  it('dispatch module is importable', async () => {
    const mod = await import('./pr_open.script')
    expect(typeof mod.run).toBe('function')
  })
})
...
describe('kit pr-check', () => {
  it('dispatch module is importable', async () => {
    const mod = await import('./pr_check.script')
    expect(typeof mod.run).toBe('function')
  })
})
