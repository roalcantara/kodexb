...
function makePhaseDecided(): WorkflowEvent {
  return {
    ...
    feature_dir: 'packages/ops/src/__tests__/fixtures/000-test',
    duration_ms: 5.2,
...
function makeHandoffWritten(): WorkflowEvent {
  return {
    ...
    feature_dir: 'packages/ops/src/__tests__/fixtures/003-sync-frecency-preserve',
    duration_ms: 42.0,
...
describe('WorkflowRunWriter (WOBS-1, WOBS-5)', () => {
  ...
  it('writes events and re-reads them byte-identical', () => {
    ...
    const writer = new WorkflowRunWriter('test-writer-rid', 'packages/ops/src/__tests__/fixtures/000-test', writerRoot)
  ...
  it('appends multiple events to the same file', () => {
    ...
    const writer = new WorkflowRunWriter(
      generateRunId('test-append'),
      'packages/ops/src/__tests__/fixtures/000-test',
      writerRoot
    )
...
describe('runs CLI (list, show, tail)', () => {
  ...
  beforeAll(() => {
    ...
    writeFileSync(
      ...
      `{"type":"phase_decided","run_id":"test-run-001","ts":"2099-06-07T12:00:00Z","feature_dir":"packages/ops/src/__tests__/fixtures/999-slug","duration_ms":5,"fileset_fingerprint":"ab","manifest_needs_handoff":false,"phase":"plan","command":"speckit.plan","focus_hint":null}\n{"type":"dispatch_invoked","run_id":"test-run-001","ts":"2099-06-07T12:00:01Z","feature_dir":"packages/ops/src/__tests__/fixtures/999-slug","duration_ms":42,"opencode_found":false,"body_bytes":0,"exit_code":0,"session_id":null}\n`
    )
  })
