...
function planWorkflow(rest: string[], env: Env, deps: { activeRun?: () => string | null }): SpecPlan {
  ...
  if (sub === 'handoff') {
    ...
    if (hc === 'generate') {
      const argv = ['bun', 'packages/exec/src/kit_verbs/handoff_generate.script.ts']
      if (feature) argv.push('--feature', feature)
      if (env.usage_focus) argv.push('--focus', env.usage_focus)
    ...
    if (hc === 'scrub') {
      const argv = ['bun', 'packages/ops/src/governance/security/handoff_scrub.script.ts']
      if (feature) argv.push('--feature', feature)
  ...
  if (sub === 'bench') {
    return {
      kind: 'spawn',
      argv: ['bun', 'packages/ops/src/metrics/harnesses/perf/perf.script.ts', 'workflow-observability']
    }
  }
  // `run` (default): delegates to spec kit next
  const feature = featureFrom(env, sub === 'run' ? rest.slice(1) : rest)
  const argv = ['bun', 'packages/ops/src/bin/spec_kit.script.ts', 'next']
  if (feature) argv.push(feature)
