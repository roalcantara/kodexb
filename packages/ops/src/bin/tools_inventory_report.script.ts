 ...
 * columns, to expose natural groupings under packages/ops/src/.
 *
 * Input : packages/ops/src/inventory/tools_file_inventory_source.csv
 *         (human-authored: path + judgment columns)
 * Output: packages/ops/src/inventory/tools_file_inventory.csv
...
// ── import graph over packages/ops/src/*.ts(x) (excludes graphify-out) ──────────────────

const tsFiles = (await Array.fromAsync(new Glob('packages/ops/src/**/*.{ts,tsx}').scan('.'))).filter(
  f => !f.startsWith('packages/ops/src/graphify-out/')
)

const topFolderOf = (p: string): string => {
  const seg = p.split('/')
  if (seg.length >= 3) return seg[1] ?? ''
  return p.replace('packages/ops/src/', '')
}
...
for (const f of tsFiles) {
  ...
  for (const spec of specs) {
    ...
    if (spec.startsWith('.')) {
      ...
      if (resolved.startsWith('packages/ops/src/')) toolImports.push(resolved)
    }
  }
...
function isGenerated(p: string, artifact: string, role: string): string {
  if (p.startsWith('packages/ops/src/graphify-out/') || p.includes('/cache/') || p.includes('/results/'))
    return 'generated'
...
function configRefs(p: string, artifact: string): { refs: string; invokedBy: string[] } {
  const folderPath = `packages/ops/src/${topFolderOf(p)}`
  ...
  // (e.g. electrobun.config.ts: `./packages/ops/src/build/x.script`). Match all variants.
...
function lsLintEntry(p: string): string {
  return new RegExp(`(^|\\n)\\s*packages/ops/src/${topFolderOf(p)}\\s*:`).test(lsLintText) ? 'yes' : 'no'
}
