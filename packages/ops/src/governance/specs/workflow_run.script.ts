...
const ALLOWLIST_PREFIXES = ['mise run', 'hk check', 'bash packages/ops/src/governance/specs/gate.sh']
...
function run(): void {
  ...
  if (args.lint) {
    const r = Bun.spawnSync(['bun', 'packages/ops/src/governance/specs/lint.script', '--strict', featureDir], {
      stdout: 'inherit',
      stderr: 'inherit'
    })
