...
import { SMOKE_FIXTURE } from './smoke_harness.script'
import { parseScopeFeature } from './spec_test.script'

describe('spec_test.script', () => {
  it('is importable', async () => {
    const mod = await import('./spec_test.script')
...
describe('spec test smoke scope', () => {
  it('uses committed smoke fixture under packages/ops/src/', () => {
    expect(SMOKE_FIXTURE).toBe('packages/ops/src/__tests__/fixtures/workflow/smoke-feature')
    expect(SMOKE_FIXTURE.startsWith('packages/ops/src/')).toBe(true)
  })
})
