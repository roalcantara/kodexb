...
describe('audit.script', () => {
  ...
  it('exits 0 for a clean feature dir (non-strict)', () => {
    ...
    try {
      const r = Bun.spawnSync(['bun', 'packages/ops/src/governance/specs/audit.script.ts', dir], {
  ...
  it('--json output parses correctly', () => {
    const { dir, cleanup } = useFeatureDir()
    try {
      const r = Bun.spawnSync(['bun', 'packages/ops/src/governance/specs/audit.script.ts', '--json', dir], {
        env: { ...process.env },
        cwd: path.resolve(import.meta.dirname, '../../../../..')
      })
      expect(r.exitCode).toBe(0)
  ...
  it('missing handoff.md causes exit 1 with --strict', () => {
    ...
    try {
      const r = Bun.spawnSync(['bun', 'packages/ops/src/governance/specs/audit.script.ts', '--strict', dir], {
        env: { ...process.env, NO_COLOR: '1' },
        cwd: path.resolve(import.meta.dirname, '../../../../..')
      })
  ...
  it('nonexistent dir exits 2', () => {
    const r = Bun.spawnSync(['bun', 'packages/ops/src/governance/specs/audit.script.ts', '/nonexistent/path'], {
      env: { ...process.env },
      cwd: path.resolve(import.meta.dirname, '../../../../..')
    })
