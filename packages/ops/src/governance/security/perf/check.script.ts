...
const RUNNERS: Record<Scope, string> = {
  secrets: path.join(ROOT, 'packages/ops/src/governance/security/perf/secrets_perf.script.ts'),
  'dependencies-noop': path.join(ROOT, 'packages/ops/src/governance/security/perf/dependencies_perf.script.ts'),
  'handoff-scrub': path.join(ROOT, 'packages/ops/src/governance/security/perf/handoff_scrub_perf.script.ts')
}
