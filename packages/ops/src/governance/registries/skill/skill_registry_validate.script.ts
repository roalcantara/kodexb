import type { DriftReport, Finding, FindingCategory, ValidatePayload } from './skill_registry.types'
import {
  ALLOWED_LOCATIONS,
  ALLOWED_TYPES,
  diffSets,
...
function addFinding(
  ...
  findings.push({ category, skill_id: skillId, message })
...
export async function runValidate(root = gitRoot()): Promise<ValidatePayload> {
  ...
  const check = (cond: boolean, category: FindingCategory, message: string, skillId?: string) => {
    if (!cond) addFinding(findings, summary, category, message, skillId)
  }
