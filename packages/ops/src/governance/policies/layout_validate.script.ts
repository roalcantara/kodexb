/**
 * Validate tracked packages/ops/src/ top-level layout against packages/ops/tools.manifest.toml.
...
function gitLsFilesTools(root: string): string[] {
  const r = Bun.spawnSync(['git', 'ls-files', 'packages/ops/src'], { cwd: root })
  if (r.exitCode !== 0) {
    throw new Error('git ls-files packages/ops/src failed')
  }
...
export function topLevelFromTracked(files: string[]): Set<string> {
  ...
  for (const f of files) {
    const rel = f.replace(/^packages\/ops\/src\//, '')
...
export function findUnmanifestedTops(manifestPaths: Set<string>, tops: Set<string>): string[] {
  ...
  for (const top of tops) {
    ...
    if (!manifestPaths.has(top)) {
      errors.push(`unmanifested packages/ops/src/${top}/`)
    }
  }
  return errors
}
...
async function main(): Promise<void> {
  ...
  const manifestPath = path.join(root, 'packages/ops/tools.manifest.toml')
  ...
  const tracked = gitLsFilesTools(root).filter(
    f => !f.includes('/benchmarks/results/') && !f.startsWith('packages/ops/src/graphify-out/')
  )
  ...
  if (errors.length > 0) {
    console.error('packages/ops/src layout validation failed:')
    for (const e of errors) console.error(`  - ${e}`)
    console.error(`manifest: ${manifestPath}`)
    process.exit(1)
  }

  console.log(`packages/ops/src layout OK (${manifestPaths.size} manifest entries, ${tops.size} top-level tracked)`)
}
