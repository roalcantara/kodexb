...
import { generateRunId, loadProfile, readStateSnapshot, WorkflowRunWriter, writeStateSnapshot } from '@kb/exec'
import {
...
const RESULTS_DIR = 'packages/ops/src/metrics/results/workflow'
const FIXTURE_PROFILE = 'packages/ops/src/__tests__/fixtures/workflow/fixture-profile.yaml'
...
function run(): void {
  ...
  try {
    ...
    persistMachineSnapshot(
      ...
      dateStr,
      Profile?.name ?? 'test',
      Profile?.schema_version ?? '009.1.0',
