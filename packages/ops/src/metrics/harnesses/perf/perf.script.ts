...
const SERVER = path.join(ROOT, 'packages/dev/src/preview/server.script')
  ...
  const actions = {
    ...
    'workflow-observability': () => {
      const child = Bun.spawnSync(
        [
          'bun',
          path.join(ROOT, 'packages/ops/src/metrics/harnesses/perf/workflow_observability_perf.script'),
    ...
    'workflow-observability extract-dataset': () => {
      ...
      const child = Bun.spawnSync(
        [
          'bun',
          path.join(ROOT, 'packages/ops/src/metrics/harnesses/perf/workflow_observability_extract_dataset.script'),
          ...args
        ],
        { cwd: ROOT, stdio: ['ignore', 'inherit', 'inherit'] }
      )
