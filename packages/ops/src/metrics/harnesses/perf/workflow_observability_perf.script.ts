...
} from './workflow_observability_perf_core.script'

const ROOT = chdirToRepoRoot()
const POPULATED_DATASET_PATH = path.join(
  ROOT,
  'packages/ops/src/metrics/fixtures/perf/workflow-observability-feature.json'
)
...
async function main(): Promise<void> {
  ...
  const { runHandoffGenerate: runGenerate } = await import('@kb/exec')
  const { runOrchestratedHandoff: runOrchestrate } = await import('@kb/exec')
