...
describe('detectPhase — transition table', () => {
  ...
  describe.each([
    ...
    },
    {
      ...
      featureDir: 'tools/__tests__/fixtures/006-unit-only-feature',
      needsHandoff: false,
  ...
  describe.each([
    {
      ...
      input: {
        featureDir: 'tools/__tests__/fixtures/003-x',
...
describe('runLint (OHW-6 AC1)', () => {
  describe.each([
    { label: 'delegates with strict feature args', featureDir: 'tools/__tests__/fixtures/foo-bar', exitCode: 0 },
    ...
    it('runs lint and returns exit code', () => {
      ...
      if (featureDir === 'tools/__tests__/fixtures/foo-bar') {
        expect(calls[0]).toEqual(['bun', 'tools/governance/specs/lint.script.ts', '--strict', featureDir])
      }
    })
  })
})
