...
Pointers:
- E2e step contracts and tag filter: \`tools/governance/registries/catalog/tag.script.ts\` (\`e2eTagExpression\`)
...
1. **\`@e2e\` filter** — bddgen e2e MUST filter \`@e2e\` via \`e2eTagExpression()\` in
   \`tools/governance/registries/catalog/tag.script.ts\`. Tag expression preserves
   \`and @e2e and not @todo\`.
2. **\`RESERVED_RUN_TAGS\`** — \`tools/governance/registries/catalog/tag.script.ts\`
