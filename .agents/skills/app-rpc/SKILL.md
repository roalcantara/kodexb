...
description: >
  ...
  must also appear in tools/preview/server.script.ts, and the Eden Treaty client
...
For the **preview server** (`tools/preview/server.script.ts`), Elysia runs over
...
- [ ] No preview-server fork — `tools/preview/server.script.ts` mounts `createRpcServer(app)` as-is
